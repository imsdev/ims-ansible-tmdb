...
class UnixFileSystem(object):
    ...
    def ascii_to_ebcdic(file_path):
        ...
        with open(path + tmp_filename, 'w+b') as f:
            request = subprocess.Popen(['iconv', '-f', 'ISO8859-1', '-t', 'IBM-1047', filename], cwd=path, stdout=f)
        stdout = request.communicate()
        request = subprocess.Popen(['mv', tmp_filename, filename], cwd=path, stdout=subprocess.PIPE)
        stdout += request.communicate()
        return stdout
    ...
    def copy_from_mvs(mvs_dsn, uss_file_path):
        ...
        request = subprocess.Popen(['cp', "//\'" + mvs_dsn + "\'", filename], cwd=path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def copy_to_mvs(uss_file_path, mvs_dsn):
        filename, path = UnixFileSystem.split_filename_and_path(uss_file_path)
        request = subprocess.Popen(['cp', filename, "//\'" + mvs_dsn + "\'" ], cwd=path, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = request.communicate()
        return {'stdout': stdout, 'stderr': stderr, 'rc': request.returncode}
...
class MVSConsole(object):
    ...
    def command(cmd_text):
        ...
        request = subprocess.Popen([wrapped_command_file.name], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = request.communicate()
...
class IPConfiguration(object):
    ...
    def retrieve(self):
        request = subprocess.Popen(['cat', "//\'" + self.dsn + "\'" ], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = request.communicate()
        return stdout
    ...
    def find_netstat_ports(self):
        request = subprocess.Popen(['netstat', '-o'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = request.communicate()
...
class Port(object):
    ...
    def get_connection_id(port):
        port = str(port)
        request = subprocess.Popen(['netstat'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = request.communicate()
