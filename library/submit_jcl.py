...
"""
    ...
    def submit(self):
        try:
            request = Popen(['submit', '-j', self.jcl_path], stdout=PIPE, stderr=PIPE)
            stdout, stderr = request.communicate()
            result = re.search(r'((?:job|JOB)[0-9]+)', stdout)
            if not result:
                raise RuntimeError('An error occurred during submission of batch job.')
    ...
    def get_rc(self, wait_seconds=2, retries=60):
        ...
        for attempt in range(retries):
            ...
            request = Popen(['./' + script, self.job_name], cwd=path, stdout=PIPE, stderr=PIPE)
            stdout, stderr = request.communicate()
